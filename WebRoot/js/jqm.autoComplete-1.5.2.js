...
(function($) {
	...
	var defaults = {
	...
	}, openXHR = {}, buildItems = function($this, data, settings) {
		var str, vclass = '';
		if (settings.klass) {
			...
			if (data) {
				...
				$.each(data, function(index, value) {
					...
					if ($.isPlainObject(value)) {
						...
						str.push('<li ' + vclass + ' data-icon='
								+ settings.icon + '><a href="' + settings.link
								+ encodeURIComponent(value.value)
								+ '" data-transition="' + settings.transition
								+ '" data-autocomplete=\''
								+ JSON.stringify(value).replace(/'/g, "&#39;")
								+ '\'>' + settings.labelHTML(value.label)
								+ '</a></li>');
					} else {
						str.push('<li ' + vclass + ' data-icon='
								+ settings.icon + '><a href="' + settings.link
		...
		$target.html('').listview('refresh').closest("fieldset").removeClass(
				"ui-search-active");
		$this.trigger("targetCleared.autocomplete");
	}, handleInput = function(e) {
		var $this = $(this), id = $this.attr("id"), text, data, settings = $this
				.jqmData("autocomplete"), element_text, re;
		...
		if (e) {
			if (e.keyCode === 38) { // up
				$('.ui-btn-active', $(settings.target)).removeClass(
						'ui-btn-active').prevAll('li.ui-btn:eq(0)').addClass(
						'ui-btn-active').length
						|| $('.ui-btn:last', $(settings.target)).addClass(
								'ui-btn-active');
			} else if (e.keyCode === 40) {
				$('.ui-btn-active', $(settings.target)).removeClass(
						'ui-btn-active').nextAll('li.ui-btn:eq(0)').addClass(
						'ui-btn-active').length
						|| $('.ui-btn:first', $(settings.target)).addClass(
								'ui-btn-active');
		...
		if (settings) {
			...
			// check if it's the same as the last onex
			...
			if (text.length < settings.minLength) {
				...
				if (settings.interval
						&& Date.now() - settings._lastRequest < settings.interval) {
					settings._retryTimeout = window.setTimeout($.proxy(
							handleInput, this), settings.interval - Date.now()
							+ settings._lastRequest);
					return;
				}
				...
				if ($.isArray(settings.source)) {
					// this function allows meta characters like +, to be
					// searched for.
					// Example would be C++
					...
					//输入匹配
					data = settings.source.sort().filter(
							function(element) {
								// matching from start, or anywhere in the
								// string?
								if (settings.matchFromStart) {
									// from start
									element_text, re = new RegExp('^'
											+ escape(text), 'i');
								} else {
									// anywhere
									element_text, re = new RegExp(escape(text),
											'i');
								}
						...
						for(var k in data){
							var index = data[k].lastIndexOf('[');
							data[k] = data[k].substr(0,index);
						}
				...
				// parameter.
				// source:function(text,callback) { mydata = [1,2];
				// callback(mydata); }
					...
					var ajax = {
						...
						beforeSend : function(jqXHR) {
							if (settings.cancelRequests) {
								if (openXHR[id]) {
									// If we have an open XML HTTP Request for
									// this autoComplete ID, abort it
									openXHR[id].abort();
								} else {
								}
								// Set this request to the open XML HTTP Request
								// list for this ID
								openXHR[id] = jqXHR;
							}
							...
							if (settings.loadingHtml) {
								// Set a loading indicator as a temporary
								// stop-gap to the response time issue
								$(settings.target).html(settings.loadingHtml)
										.listview('refresh');
								$(settings.target).closest("fieldset")
										.addClass("ui-search-active");
							}
						},
						...
						complete : function() {
							// Clear this ID's open XML HTTP Request from the
							// list
							if (settings.cancelRequests) {
								openXHR[id] = null;
							}
		...
		init : function(options) {
			...
			return el.unbind("keyup.autocomplete").bind("keyup.autocomplete",
					handleInput).next('.ui-input-clear').bind('click',
					function() {
